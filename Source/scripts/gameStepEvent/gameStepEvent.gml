function gameStepEvent() {
	...
	with oMovingSolid {
	  ...
	  with oCharacter {calculateCollisionBounds();}    //calculate the collision bounds of the character -- we'll need it later
	      ...
	      for (x=x; x<mstXPrev+xVelInteger; x+=1;) {
	        for(i=0; i<oGame.players_length; i+=1;) {
	          if (viscidTop and isCollisionCharacterTop(1,oGame.players[i]) and ((oGame.players[i]).viscidMovementOk=1 or (oGame.players[i]).viscidMovementOk=2)) {
	            ...
	            with oGame.players[i] {collision=isCollisionRight(1);}
	      ...
	      for (x=x; x>mstXPrev+xVelInteger; x-=1;) {
	        for (i=0; i<oGame.players_length; i+=1;) {
	          if (viscidTop and isCollisionCharacterTop(1,oGame.players[i]) and (oGame.players[i].viscidMovementOk=1 or oGame.players[i].viscidMovementOk=2)) {
	            ...
	            with oGame.players[i] {collision=isCollisionLeft(1);}
	            if (oGame.players[i].collision) {
	              breakNow=1;
	              break;
	            }
	      ...
	      for (y=y; y<mstYPrev+yVelInteger; y+=1;) {
	        for (i=0; i<oGame.players_length; i+=1;) {
	          if (viscidTop and isCollisionCharacterTop(2,oGame.players[i])) {
	            ...
	            with oGame.players[i] {collision=isCollisionBottom(1);}
	            if (oGame.players[i].collision) {
	              breakNow=1;
	              break;
	            }
	      ...
	      for (y=y; y>mstYPrev+yVelInteger; y-=1;) {
	        for (i=0; i<oGame.players_length; i+=1;) {
	          ...
	          if (isCollisionCharacterTop(1,oGame.players[i])) { 
	            with oGame.players[i] {collision=isCollisionTop(1);}
	            if (oGame.players[i].collision) {
	              breakNow=1;
	              break;
	            }
